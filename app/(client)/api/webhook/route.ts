...
async function createOrderInSanity(
  ...
  const {
    ...
    payment_intent,
    total_details,
    shipping_details, // <--- CAMBIO: Extraemos los datos de envío de Stripe
  } = session;

  const { orderNumber, customerName, customerEmail, clerkUserId } =
    metadata as unknown as Metadata;

  // Mapeamos los campos de dirección de Stripe a variables legibles
  const stripeAddress = shipping_details?.address;
  const receiverName = shipping_details?.name;
  ...
  // Creamos la orden en Sanity usando los datos de envío de Stripe
  const order = await backendClient.create({
    ...
    invoice: invoice
      ...
      : null,
    // CAMBIO: Ahora usamos la dirección capturada por Stripe Checkout
